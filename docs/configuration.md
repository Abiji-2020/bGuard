...
A couple of features use an "init/loading strategy" which configures behavior at bGuard startup.  
This applies to all of them. The default strategy is blocking.

| strategy    | Description                                                                                                                                                     |
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| blocking    | Initialization happens before DNS resolution starts. Any errors are logged, but bGuard continues running if possible.                                           |
| failOnError | Like blocking but bGuard will exit with an error if initialization fails.                                                                                       |
| fast        | bGuard starts serving DNS immediately and initialization happens in the background. The feature requiring initialization will enable later on (if it succeeds). |
...
To resolve a DNS query, bGuard needs external public or private DNS resolvers. bGuard supports DNS resolvers with
...
!!! hint
    ...
    Per default bGuard uses the `parallel_best` upstream strategy where bGuard picks 2 random resolvers from the list for each query and
...
!!! note
    bGuard needs at least the configuration of the **default** group with at least one upstream DNS server. This group will be used as a fallback, if no client
...
bGuard will wait 2 seconds (default value) for the response from the external upstream DNS server. You can change this
...
bGuard supports different upstream strategies (default `parallel_best`) that determine how and to which upstream DNS servers requests are forwarded.

Currently available strategies:

- `parallel_best`: bGuard picks 2 random (weighted) resolvers from the upstream group for each query and returns the answer from the fastest one.  
...
- `random`: bGuard picks one random (weighted) resolver from the upstream group for each query and if successful, returns its response.  
...
- `strict`: bGuard forwards the request in a strict order. If the first upstream does not respond, the second is asked, and so on.
...
In domain environments, it may be useful to only response to FQDN requests. If this option is enabled bGuard respond immediately
...
* `$INCLUDE` - includes another zone file relative to the bGuard executable
...
With parameter `filterUnmappedTypes = true` (default), bGuard will filter all queries with unmapped types, for example:
...
bGuard can try to resolve a user-friendly client name from the IP address or server URL (DoT and DoH). This is useful
...
DoH URL: `https://bGuard.example.com/dns-query/alice` -> request's client name is `alice`

### Resolving client name from IP address

bGuard uses rDNS to retrieve client's name. To use this feature, you can configure a DNS server for client lookup (
...
bGuard can use lists of domains and IPs to block (e.g. advertisement, malware,
...
group, which bGuard adult sites.
...
If full-qualified domain name is used (for example "myclient.ddns.org"), bGuard will try to resolve the IP address (A and AAAA records) of this domain.
...
values are maintained by domain owners, server administrators etc. bGuard caches the answers from all resolved queries
in own cache in order to avoid repeated requests. This reduces the DNS traffic and increases the network speed, since
bGuard can serve the result immediately from the cache.
...
bGuard can synchronize its cache and blocking state between multiple instances through redis.
...
| redis.required           | bool            | no        | false         | Connection is required for bGuard to start                          |
...
bGuard can expose various metrics for prometheus. To use the prometheus feature, the HTTP listener must be enabled (
...
!!! hint
    If not defined, bGuard will log all available information
...
See [Wiki - Configuration of HTTPS](https://github.com/Abiji-2020/bGuard/wiki/Configuration-of-HTTPS-for-DoH-and-Rest-API)
...
!!! example
    ...
    ```yaml
    - https://example.com/a/source # bGuard will download and parse the file
    - /a/file/path # bGuard will read the local file
    - | # bGuard will parse the content of this multi-line string
...
To keep source contents up-to-date, bGuard can periodically refresh and reparse them. Default period is
...
bGuard downloads and processes sources concurrently. This allows limiting how many can be processed in the same time.  
